...
export function App() {
  ...
  const range = 30;
  return (
    <Canvas className="mx-auto aspect-square w-full max-w-3xl" camera={{ position: [0, 0, 20], fov: 50 }}>
...
function Shoe({ random, color = new THREE.Color(), ...props }) {
  ...
  useFrame((state) => {
    ...
    ref.current.scale.x =
      ref.current.scale.y =
      ref.current.scale.z =
        THREE.MathUtils.lerp(ref.current.scale.z, hovered ? 1.5 : 1, 0.1);
    ref.current.color.lerp(color.set(hovered ? "#fe5" : "white"), hovered ? 1 : 0.1);
  });
  return (
    <group {...props}>
      <Instance
